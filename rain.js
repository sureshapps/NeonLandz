...
class RainSplat {
  constructor(rainDrop) {
    this.particles = [];
    const numberOfParticles = 5;//rainDrop.length / 3;
    for (let i = 2; i < numberOfParticles; ++i) {
      const x = rainDrop.pos.x;
      const y = 10;//rainDrop.pos.y;
      const vx = random(-2, 1);
      const vy = random(1, -2 * rainDrop.vel.y/6);
...
function setup() {
  let c = createCanvas(document.querySelector(".hero").offsetWidth,
